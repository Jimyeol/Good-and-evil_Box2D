...
bool cKey::KeyUp(int vk)
{
	if( KeyUpChk(vk) )
	{
		...
		else if( vk == 32 && m_KeyDownChk[vk-55] )			//VK_SPACE 
		{
			m_KeyDownChk[vk-55] = false;
			return true;
		}
...
bool cKey::LongTimeKeyDown( int vk, float time )
{
	time *= 1000 ;
...
bool cKey::DoubleKeyDown( int vk, float time )
{
	static DWORD			DelayTime = GetTickCount() ;	// 두번 입력 완료시 다시 입력할때
	//딜레이를 주기 위한 시간 변Soo
